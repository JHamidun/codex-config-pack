...
```bash
INIT=$(node "${CODEX_PACK_ROOT}/library/get-shit-done/bin/gsd-tools.cjs" init progress)
...
- `STATE=$(node "${CODEX_PACK_ROOT}/library/get-shit-done/bin/gsd-tools.cjs" state-snapshot)`
...
- Use `summary-extract` for efficient parsing:
  ```bash
  node "${CODEX_PACK_ROOT}/library/get-shit-done/bin/gsd-tools.cjs" summary-extract <path> --fields one_liner
  ```
...
PROGRESS_BAR=$(node "${CODEX_PACK_ROOT}/library/get-shit-done/bin/gsd-tools.cjs" progress bar --raw)
```
...
```bash
DEBT=$(node "${CODEX_PACK_ROOT}/library/get-shit-done/bin/gsd-tools.cjs" audit-uat --raw 2>/dev/null)
```
...
```bash
PHASE_SECTION=$(node "${CODEX_PACK_ROOT}/library/get-shit-done/bin/gsd-tools.cjs" roadmap get-phase "${CURRENT_PHASE}" 2>/dev/null)
...
```bash
NEXT_PHASE_SECTION=$(node "${CODEX_PACK_ROOT}/library/get-shit-done/bin/gsd-tools.cjs" roadmap get-phase "$((Z+1))" 2>/dev/null)
