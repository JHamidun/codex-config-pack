...
- **Global scope**: `${CODEX_PACK_ROOT}/library/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `${CODEX_PACK_ROOT}/library/notes/`)
...
2. Glob `${CODEX_PACK_ROOT}/library/notes/*.md` (if directory exists) — global notes
...
Global (${CODEX_PACK_ROOT}/library/notes/):
...
2. **No `.planning/`**: Falls back to global `${CODEX_PACK_ROOT}/library/notes/` — works in any directory
