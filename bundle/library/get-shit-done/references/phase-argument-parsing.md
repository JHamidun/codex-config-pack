...
```bash
PHASE_INFO=$(node "${CODEX_PACK_ROOT}/library/get-shit-done/bin/gsd-tools.cjs" find-phase "${PHASE}")
```
...
```bash
PHASE_CHECK=$(node "${CODEX_PACK_ROOT}/library/get-shit-done/bin/gsd-tools.cjs" roadmap get-phase "${PHASE}" --pick found)
...
```bash
PHASE_DIR=$(node "${CODEX_PACK_ROOT}/library/get-shit-done/bin/gsd-tools.cjs" find-phase "${PHASE}" --raw)
```
