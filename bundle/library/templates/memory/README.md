...
```
${CODEX_PACK_ROOT}/library/projects/<project>/memory/
...
stores it in a local SQLite DB at `${CODEX_PACK_ROOT}/library/memory-graph/graph.db` (offline,
...
python ${CODEX_PACK_ROOT}/library/scripts/memory_graph.py search deploy

# Direct neighbors of a node (optional depth, default 1)
python ${CODEX_PACK_ROOT}/library/scripts/memory_graph.py neighbors deploy-config-service-a 2

# Shortest path between two nodes (BFS)
python ${CODEX_PACK_ROOT}/library/scripts/memory_graph.py path project-alpha quality-gates

# supersedes chain: what a note replaced and what replaced it
python ${CODEX_PACK_ROOT}/library/scripts/memory_graph.py timeline old-decision-note

# Most-connected nodes (hubs), optional N
python ${CODEX_PACK_ROOT}/library/scripts/memory_graph.py hubs 15
```
...
python ${CODEX_PACK_ROOT}/library/scripts/memory_graph.py orphans

# [[links]] pointing at notes that don't exist yet — candidates to create
python ${CODEX_PACK_ROOT}/library/scripts/memory_graph.py dangling

# Combined gap analysis: orphans + dangling + stale hubs + unmarked-superseded
python ${CODEX_PACK_ROOT}/library/scripts/memory_graph.py gaps        # optional [stale_days], default 45
```
