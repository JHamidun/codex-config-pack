...
```bash
python ${CODEX_PACK_ROOT}/library/tools/search_chats.py index --force
```
...
- Chats are stored in `${CODEX_PACK_ROOT}/library/projects/` as JSONL files
- Archived chats in `${CODEX_PACK_ROOT}/library/projects/*/archive/`
- Both active and archived sessions are indexed
- Incremental: only new/modified files are re-indexed
- Database: `${CODEX_PACK_ROOT}/library/chats.db` (SQLite FTS5)
