...
```python
import sys
sys.path.append('${CODEX_PACK_ROOT}/library/tools')
...
```bash
# Basic research
python ${CODEX_PACK_ROOT}/library/tools/perplexity_helper.py research "What are AI agents?"

# Market research
python ${CODEX_PACK_ROOT}/library/tools/perplexity_helper.py market "AI automation tools"

# Fact check
python ${CODEX_PACK_ROOT}/library/tools/perplexity_helper.py fact-check "Claude 4 has 1M context"

# Tech analysis
python ${CODEX_PACK_ROOT}/library/tools/perplexity_helper.py tech "FastAPI" --depth comprehensive

# Compare sources
python ${CODEX_PACK_ROOT}/library/tools/perplexity_helper.py compare "MCP protocol" --sources "anthropic.com" "github.com"
```
...
2. **Пассаж присутствует** — если из источника взята цифра/цитата/имя, re-fetch страницы (WebFetch) и убедись, что точное значение реально есть в тексте. «Страница про эту тему, значит цифра там есть» — не проверка. Точного совпадения нет → цитату выбросить или пометить UNVERIFIED.

Fabricated citations — самый частый режим отказа LLM-ресёрча; «на глаз» этот класс не ловится, только механически. Стоит дёшево (curl + один WebFetch на источник), закрывает главный риск.
...
- **Helper код:** `${CODEX_PACK_ROOT}/library/tools/perplexity_helper.py`
