...
1. **Local config** — check `config/brand-voice.json` in your Claude config dir (личный файл, в пак не входит; готовый шаблон рядом — `${CODEX_PACK_ROOT}/library/config/brand-voice.example.json`, скопируй и заполни)
...
2. **Notion MCP** — search for "Brand Voice" or "Style Guide" pages
   - Use `mcp__notion__search` with queries: `"Brand Voice"`, `"Style Guide"`, `"Brand Guidelines"`
...
- Commands: `mcp__notion__search`, `mcp__notion__get_page`, `mcp__notion__create_page`
...
- Commands: `mcp__slack__send_message` with channel name from user input
