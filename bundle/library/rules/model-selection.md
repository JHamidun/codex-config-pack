...
> уже Opus 5. Пиши `model: inherit-current-codex-model`, а не номер версии.

> **КАНОН (config/models.md):** движок text-субагентов = **Fable 5.1** (`model: inherit-current-codex-model`), числом не ограничивать; Fable упал на лимите → подхватить Opus (resume + смена model). Алиас **`opus`** = основная сессия / оркестратор. Уровни ниже (opus/standard/light) выбирают СЛОЖНОСТЬ задачи — глубину промпта и контекст — а НЕ движок воркера.
...
| **light-level** | Короткий промпт, низкий effort; для 10+ параллельных простых прогонов допустим `model: inherit-current-codex-model` |
...
```python
Task(subagent_type="general-purpose", model: inherit-current-codex-model, prompt="...")  # text-воркер (ВСЕГДА дефолт)
Task(subagent_type="general-purpose", model: inherit-current-codex-model, prompt="...")  # массовое простое (10+ параллельных)
Task(subagent_type="general-purpose", model: inherit-current-codex-model,  prompt="...")  # оркестратор / подхват после лимита Fable
```

- В frontmatter агентов: `model: inherit-current-codex-model` (канон, проставлен во всех agents/; исключение — orchestrator.md = opus).
