...
python ${CODEX_PACK_ROOT}/library/tools/context7_docs.py search fastapi
python ${CODEX_PACK_ROOT}/library/tools/context7_docs.py search sqlalchemy

# Step 2 — fetch the docs for the area you are about to touch
python ${CODEX_PACK_ROOT}/library/tools/context7_docs.py docs /tiangolo/fastapi --topic "dependency injection" --max-chars 8000
python ${CODEX_PACK_ROOT}/library/tools/context7_docs.py docs /sqlalchemy/sqlalchemy --topic "async session" --max-chars 8000
```
...
async def test_create_user_duplicate_email(client: AsyncClient, auth_headers: dict):
    payload = {"email": "user@example.com", "name": "First"}
