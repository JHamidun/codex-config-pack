...
1. Run `last30days` with `--agent --quick` flag via Bash (timeout 300s):
   ```bash
   python ${CODEX_PACK_ROOT}/library/skills/last30days/scripts/last30days.py --agent --quick --topic "AI"
   ```
...
Read skills:
- `${CODEX_PACK_ROOT}/library/skills/heygen/SKILL.md`
- `${CODEX_PACK_ROOT}/library/skills/video-generation/SKILL.md`
- `${CODEX_PACK_ROOT}/library/skills/nano-banana-pro/SKILL.md`
...
```bash
# Environment — всё из ${CODEX_PACK_ROOT}/library/.credentials.master.env
HEYGEN_API_KEY
...
3. If video generation fails, apply Ken Burns effect on the reference image:
   ```bash
   python ${CODEX_PACK_ROOT}/library/skills/video-editor/video_editor.py ken-burns reference_NN.png --duration 6 -o scene_NN.mp4
   ```
...
Read skills:
- `${CODEX_PACK_ROOT}/library/skills/elevenlabs/SKILL.md`
- `${CODEX_PACK_ROOT}/library/skills/heygen/SKILL.md` (Starfish TTS section)
...
```bash
# Голос — СВОЙ: $ELEVENLABS_VOICE_ID_RU (${CODEX_PACK_ROOT}/library/.credentials.master.env).
...
python ${CODEX_PACK_ROOT}/library/skills/video-generation/scripts/elevenlabs_voiceover.py \
...
python ${CODEX_PACK_ROOT}/library/skills/video-editor/video_editor.py music-pool
```
...
```bash
python ${CODEX_PACK_ROOT}/library/skills/video-editor/scripts/ui_sfx.py --list      # каталог
python ${CODEX_PACK_ROOT}/library/skills/video-editor/scripts/ui_sfx.py pop -o sfx_pop.wav
```
...
Read skills:
- `${CODEX_PACK_ROOT}/library/skills/video-editor/SKILL.md`
- `${CODEX_PACK_ROOT}/library/skills/submagic/SKILL.md`
- `${CODEX_PACK_ROOT}/library/skills/video-generation/SKILL.md` (ASS captions section)

#### 5.1 Concatenation

Assemble all scenes in order:
```bash
python ${CODEX_PACK_ROOT}/library/skills/video-editor/video_editor.py concat \
...
python ${CODEX_PACK_ROOT}/library/skills/video-editor/video_editor.py ducking \
  assembled.mp4 \
...
1. **WhisperX karaoke captions** (free, local, лучшее word-level попадание на русском):
   ```bash
   python ${CODEX_PACK_ROOT}/library/skills/video-editor/scripts/karaoke_captions.py \
...
python ${CODEX_PACK_ROOT}/library/skills/video-editor/video_editor.py thumbnail \
...
отдельного скилла-обёртки в паке нет). Прочитай `${CODEX_PACK_ROOT}/library/commands/youtube-upload.md` и выполни
описанный там код.
...
```bash
test -f ${CODEX_PACK_ROOT}/library/.youtube-oauth-token.json && echo "TOKEN_EXISTS" || echo "NO_TOKEN"
```

Нет токена — нужен OAuth client type *Desktop app* из Google Cloud Console (с включённым
YouTube Data API v3), сохранённый в `${CODEX_PACK_ROOT}/library/.youtube-client-secrets.json`. Первый вызов
...
- **API Keys**: Always load from `${CODEX_PACK_ROOT}/library/.credentials.master.env` via `os.getenv()`
