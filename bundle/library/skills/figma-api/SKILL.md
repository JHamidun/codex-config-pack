...
3. Положи в переменные окружения или в свой `${CODEX_PACK_ROOT}/library/.credentials.master.env`
   (образец — `${CODEX_PACK_ROOT}/library/templates/.credentials.master.env.example`):
...
Готовый CLI без написания кода — `${CODEX_PACK_ROOT}/library/tools/figma_api.py`
...
```bash
python ${CODEX_PACK_ROOT}/library/tools/figma_api.py me         # проверить токен
python ${CODEX_PACK_ROOT}/library/tools/figma_api.py projects   # список проектов команды
```
...
Сделай себе табличку и держи её **вне пака** (`${CODEX_PACK_ROOT}/library/figma-ids.md`), чтобы она
...
Заполняется выводом `python ${CODEX_PACK_ROOT}/library/tools/figma_api.py projects`.
