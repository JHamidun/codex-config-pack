...
```bash
SKILL_DIR="${CODEX_PACK_ROOT}/library/skills/check-skill-solo"
...
```bash
mkdir -p "${CODEX_PACK_ROOT}/library/skills/check-skill-solo"
cat > "${CODEX_PACK_ROOT}/library/skills/check-skill-solo/.env.local" <<'EOF'
# Local only — never shared. Leave blank to use your CLI's interactive login instead.
OPENAI_API_KEY=""
GEMINI_API_KEY=""
EOF
chmod 600 "${CODEX_PACK_ROOT}/library/skills/check-skill-solo/.env.local"
```
...
- Keys are read only from `${CODEX_PACK_ROOT}/library/skills/check-skill-solo/.env.local`, never from this file.
