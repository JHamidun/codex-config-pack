---
name: "book-post"
description: "Главы нон-фикшн книги: source-anchored writing, case-driven HBR-структура, конвейер из 9 стадий (writer → факт-чек → голос → корректура → редактура). Триггеры: «напиши главу», «пролог книги», «интерлюдия», «продолжаем книгу»."
---
...
```bash
python ${CODEX_PACK_ROOT}/library/skills/leak-scan/scripts/leak_scan.py "$BOOK_ROOT/chapters/<slug>"
```

Ищет ID мессенджеров, почты, ключи, IP серверов, локальные пути, а по твоему
`${CODEX_PACK_ROOT}/library/leak-scan-identity.json` — ещё и твои личные имена и домены (см. навык `leak-scan`).
...
1. `${CODEX_PACK_ROOT}/library/voice-sample.md` — **голос автора**: его собственные тексты и правила. Это источник истины.
2. `$BOOK_ROOT/STYLE_GUIDE.md` — решения по этой конкретной книге (термины, обращение, тон).
3. `${CODEX_PACK_ROOT}/library/skills/author-voice/anti-ai-tells.md` — паттерны ИИ-почерка с правилом приоритета.
4. `${CODEX_PACK_ROOT}/library/skills/author-voice/writing-craft.md` и `writing-craft-v2.md` — ремесло текста.

Вспомогательные (по необходимости): `${CODEX_PACK_ROOT}/library/skills/de-ai-ify/SKILL.md`.
...
cd ${CODEX_PACK_ROOT}/library/skills/book-post/scripts
