...
KEYCHAIN_KEYS = (
    ...
    "GOOGLE_GENAI_API_KEY", "SCRAPER_API_KEY", "APIFY_API_TOKEN",
...
def get_config(policy: ConfigLoadPolicy | None = None) -> dict[str, Any]:
    ...
    # Backward-compat: ScraperVendor' own examples and tutorials use the
    ...
    # don't silently end up with has_scraper-vendor=False. Canonical name
    ...
    if not config.get('SCRAPER_API_KEY'):
        ...
        if legacy:
            config['SCRAPER_API_KEY'] = legacy

    # Multi-key rotation: comma-separated SCRAPER_API_KEY round-robins
    # via random.choice per run. Originally added in #268, accidentally dropped
    # in v3.0.6, restored here.
    sc_key_raw = config.get('SCRAPER_API_KEY') or ''
    if ',' in sc_key_raw:
        import random
        sc_keys = [k.strip() for k in sc_key_raw.split(',') if k.strip()]
        config['SCRAPER_API_KEY'] = random.choice(sc_keys) if sc_keys else ''
...
def get_reddit_source(config: dict[str, Any]) -> str | None:
    ...
    Returns: 'scraper-vendor' or None
    """
...
def is_youtube_comments_available(config: dict[str, Any]) -> bool:
    ...
    only ever existed to gate ScraperVendor credit spend, and there is none to
    gate. ``EXCLUDE_SOURCES=youtube_comments`` remains the off-switch.

    Without yt-dlp, the legacy ScraperVendor path still applies: it requires
    SCRAPER_API_KEY AND ``youtube_comments`` in ``INCLUDE_SOURCES``
...
def is_tiktok_comments_available(config: dict[str, Any]) -> bool:
    ...
    Requires SCRAPER_API_KEY AND tiktok_comments in INCLUDE_SOURCES.
...
def is_instagram_comments_available(config: dict[str, Any]) -> bool:
    ...
    Requires SCRAPER_API_KEY AND instagram_comments in INCLUDE_SOURCES.
    Mirrors the youtube_comments / tiktok_comments opt-in pattern. Comments are
    fetched via ScraperVendor (GET /v2/instagram/post/comments) with each
...
def is_youtube_sc_available(config: dict[str, Any]) -> bool:
    """Check if ScraperVendor YouTube search fallback is available.

    Used when yt-dlp is not installed or fails.
    """
    return bool(config.get('SCRAPER_API_KEY'))
...
def is_tiktok_available(config: dict[str, Any]) -> bool:
    """Check if TikTok source is available (ScraperVendor or legacy Apify).

    Returns True if SCRAPER_API_KEY or APIFY_API_TOKEN is set.
    """
    return bool(config.get('SCRAPER_API_KEY') or config.get('APIFY_API_TOKEN'))


def get_tiktok_token(config: dict[str, Any]) -> str:
    """Get TikTok API token, preferring ScraperVendor over legacy Apify."""
    return config.get('SCRAPER_API_KEY') or config.get('APIFY_API_TOKEN') or ''
...
def is_threads_available(config: dict[str, Any]) -> bool:
    ...
    Returns True when SCRAPER_API_KEY is set. This is an availability
...
def is_instagram_available(config: dict[str, Any]) -> bool:
    """Check if Instagram source is available (ScraperVendor).

    Returns True if SCRAPER_API_KEY is set.
...
def get_instagram_token(config: dict[str, Any]) -> str:
    """Get Instagram API token (same ScraperVendor key as TikTok)."""
    return config.get('SCRAPER_API_KEY') or ''
...
def is_pinterest_available(config: dict[str, Any]) -> bool:
    ...
    Returns True when SCRAPER_API_KEY is set AND 'pinterest' is in
...
def get_pinterest_token(config: dict[str, Any]) -> str:
    """Get Pinterest API token (same ScraperVendor key as TikTok/Instagram)."""
    return config.get('SCRAPER_API_KEY') or ''
