...
## План/факт: декомпозиция отклонений (RU)

Обзоры выше отвечают «что изменилось». Этот раздел — «почему», разложением на драйверы. Порт `finance:variance-analysis`, US-GAAP/SOX-часть выброшена.

### Разложение на объём × цену × микс

Работает для любой метрики вида «количество × ставка»: MRR = платящие × ARPU; выручка академии = потоков × мест × цена; консалтинг = проектов × средний чек; косты LLM = запросов × цена за 1К токенов.

```
Отклонение = Факт − План
Эффект объёма = (Объём_факт − Объём_план) × Цена_план
Эффект цены   = (Цена_факт  − Цена_план)  × Объём_факт
Проверка: эффект объёма + эффект цены = полное отклонение
```

Микс отделяется, когда сегменты имеют разную экономику (corp vs individual в YourProduct, годовые vs месячные подписки): `эффект микса = Цена_план × Объём_план × (Доля_факт − Доля_план)`. Классический случай: выручка выросла, средний чек упал — вырос дешёвый сегмент; без разделения микса это ошибочно читается как «уронили цену».

Для расходов, где «цена × объём» не работает, раскладывай по природе драйвера: люди/подрядчики · косты, растущие от объёма (инфра, API, эквайринг) · дискреционные (реклама, мероприятия) · контрактные (аренда, лицензии) · разовые · сдвиг периода (заплатили раньше/позже плана).

### Порог существенности

Отклонение расследуется, если пробит **либо** денежный, **либо** процентный порог. Порог ужесточается по мере сближения базы сравнения с реальностью:

| База сравнения | Ориентир по % | Почему |
|---|---|---|
| Факт vs годовой план | ~10% | план ставился давно, дрейф нормален |
| Факт vs предыдущий период | ~15% | сезонность и шум |
| Факт vs свежий прогноз | ~5% | прогнозу месяц — большая дельта = сломанная модель |
| Месяц к месяцу | ~20% | самый шумный срез |

Денежный порог задаётся от масштаба бизнеса (обычная практика — 0,5–1% выручки), а не абсолютной цифрой из шаблона. Приоритет расследования: самая большая абсолютная дельта → самая большая процентная (часто = ошибка в данных, а не в бизнесе) → отклонение против ожидаемого направления → метрика, которая была в норме и сорвалась → отклонение, растущее третий период подряд.

### Водопад (bridge)

```
База (план/прошлый период)
  +/− драйвер 1 … драйвер N   (5–8 максимум, остальное в «прочее»)
= Факт
Обязательная сверка: база + сумма драйверов = факт. Не сходится — декомпозиция неполная, не публикуй.
```

Рядом — таблица «драйвер · сумма · % от отклонения · накопленным итогом». Доли отдельных драйверов могут превышать 100%, если есть встречные — это нормально, не признак ошибки.

### Нарратив к отклонению

Формат: `[метрика]: [благоприятное/неблагоприятное] отклонение X (Y%) против [база] за [период]` → **драйвер** (2–3 предложения с количественной раскладкой) → **дальше** (разовое / продолжится / улучшается / ухудшается) → **действие** (ничего / наблюдать / копать / пересобрать прогноз).

Антипаттерны, которые убивают ценность обзора: «выручка выше плана, потому что выросла выручка» (круг) · «расходы были повышенные» (какие, почему?) · «тайминг» без указания, что именно сдвинулось и когда вернётся · «разовое» без расшифровки, что это было · «прочие мелкие статьи» на существенную сумму · рассказ только про самый крупный драйвер при наличии крупных встречных.

### Три базы и точность прогноза

Держи три колонки: **план** (ставится раз и не двигается — по нему меряется год), **прогноз** (пересобирается ежемесячно — по нему управляется операционка), **факт**. Дельта «прогноз vs план» показывает не работу команды, а то, как менялись ожидания.

Точность прогноза: `1 − |Факт − Прогноз| / |Факт|`, средняя по периодам = MAPE. Копи её — она диагностирует систематический перекос: стабильно благоприятные отклонения = план занижен (перестраховка), стабильно неблагоприятные = завышен либо проблема с исполнением, растущие неблагоприятные = деградация, сужающиеся к концу года = нормальное обучение модели, скачущие = либо бизнес непредсказуем, либо методика прогноза плохая.
