...
**Перед началом прочитай `${CODEX_WORKSPACE}/.codex-context/business-context.md`** — разделы «Продукт» (как называется бренд, за какими словами его должны находить), «Позиционирование» (в какой категории хочешь, чтобы тебя искали, и с кем сравнивают) и «ICP» (кто задаёт вопрос модели). Файла нет — заведи из `${CODEX_PACK_ROOT}/library/templates/business-context.md`.
...
```bash
cd ${CODEX_PACK_ROOT}/library/skills/geo-aeo-ru/scripts
