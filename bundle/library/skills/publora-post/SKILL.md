...
1. **MCP**: сервер `publora` (`https://mcp.publora.com`, `Authorization: Bearer $API_KEY
2. **REST fallback** (если MCP недоступен или нужны `platformSettings`): `https://api.publora.com/api/v1`, заголовок `x-publora-key: $PUBLORA_API_KEY
