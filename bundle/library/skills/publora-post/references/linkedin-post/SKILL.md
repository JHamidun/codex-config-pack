...
Тот же `PUBLORA_API_KEY` покрывает не только посты, но и комменты с реакциями. URL-парсер и готовые Python-обёртки лежат в `${CODEX_PACK_ROOT}/library/skills/publora-post/scripts/lib/`.
...
python ${CODEX_PACK_ROOT}/library/skills/publora-post/scripts/lib/url_parser.py \
...
```python
import sys
sys.path.insert(0, "${CODEX_PACK_ROOT}/library/skills/publora-post/scripts")
...
```bash
python ${CODEX_PACK_ROOT}/library/skills/publora-post/scripts/post_comment.py \
