...
```bash
python ${CODEX_PACK_ROOT}/library/tools/llms_txt.py https://docs.example.com --full --save ./llms_src
```
...
`${CODEX_PACK_ROOT}/library/scripts/text_sanitize.py` до записи в JSON: вырезает zero-width символы,
...
Проверить отдельный файл вручную: `python ${CODEX_PACK_ROOT}/library/scripts/text_sanitize.py doc.txt --scan`
...
**Команды.** Движок в пак не входит: `git clone https://github.com/VectifyAI/PageIndex ${CODEX_PACK_ROOT}/library/mcps/pageindex`, поднять venv по их README и положить рядом свою обёртку `pi.py` с подкомандами index/list/tree/pages/ask. Ключ берётся из `${CODEX_PACK_ROOT}/library/.credentials.master.env`:

```bash
# Интерпретатор venv лежит по-разному: Windows — Scripts/python.exe,
# macOS и Linux — bin/python. Берём тот, который реально есть.
VENV=${CODEX_PACK_ROOT}/library/mcps/pageindex/.venv
PY="$VENV/bin/python"; [ -x "$PY" ] || PY="$VENV/Scripts/python.exe"
PI=${CODEX_PACK_ROOT}/library/mcps/pageindex/pi.py
