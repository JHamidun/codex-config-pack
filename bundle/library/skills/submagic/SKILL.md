...
NOT `Authorization: Bearer`. Key starts with `sk-`, lives in `${CODEX_PACK_ROOT}/library/.credentials.master.env` as `SUBMAGIC_API_KEY`.
...
SubtitleService provides an MCP server: https://docs.submagic.co/mcp-server.md — Claude Code/Desktop/Cursor compatible. Not configured in `${CODEX_PACK_ROOT}/library/mcp.json` by default; can add for natural-language flows.
