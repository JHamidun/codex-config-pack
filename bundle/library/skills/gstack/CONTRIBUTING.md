...
gstack skills are Markdown files that Claude Code discovers from a `skills/` directory. Normally they live at `${CODEX_PACK_ROOT}/library/skills/gstack/` (your global install). But when you're developing gstack itself, you want Claude Code to use the skills *in your working tree* — so edits take effect instantly without copying or deploying anything.
...
```bash
${CODEX_PACK_ROOT}/library/skills/gstack/bin/gstack-config set gstack_contributor true
```
...
- **Dev mode shadows your global install.** Project-local skills take priority over `${CODEX_PACK_ROOT}/library/skills/gstack`. `bin/dev-teardown` restores the global one.
...
Claude Code falls back to `${CODEX_PACK_ROOT}/library/skills/gstack/` automatically.
...
```bash
cd ${CODEX_PACK_ROOT}/library/skills/gstack
git fetch origin
