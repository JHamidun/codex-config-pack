...
- Stale servers from global install no longer shadow local changes — removed legacy `${CODEX_PACK_ROOT}/library/skills/gstack` fallback from `resolveServerScript()`
...
- Legacy fallback to `${CODEX_PACK_ROOT}/library/skills/gstack/browse/src/server.ts`
