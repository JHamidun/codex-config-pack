---
name: "gstack-upgrade"
description: "Upgrade gstack to the latest version. Detects global vs vendored install, runs the upgrade, and shows what's new. "
---
...
[ -z "$_AUTO" ] && _AUTO=$(bash ${CODEX_PACK_ROOT}/library/skills/gstack/bin/gstack-config get auto_upgrade 2>/dev/null || true)
...
```bash
bash ${CODEX_PACK_ROOT}/library/skills/gstack/bin/gstack-config set auto_upgrade true
```
...
**If "Never ask again":**
```bash
bash ${CODEX_PACK_ROOT}/library/skills/gstack/bin/gstack-config set update_check false
```
Tell user: "Update checks disabled. Run `bash ${CODEX_PACK_ROOT}/library/skills/gstack/bin/gstack-config set update_check true` to re-enable."
Continue with the current skill.

### Step 2: Detect install type

```bash
if [ -d "${CODEX_PACK_ROOT}/library/skills/gstack/.git" ]; then
  INSTALL_TYPE="global-git"
  INSTALL_DIR="${CODEX_PACK_ROOT}/library/skills/gstack"
  ...
  INSTALL_DIR=".claude/skills/gstack"
elif [ -d "${CODEX_PACK_ROOT}/library/skills/gstack" ]; then
...
```bash
bash ${CODEX_PACK_ROOT}/library/skills/gstack/bin/gstack-update-check --force
```
