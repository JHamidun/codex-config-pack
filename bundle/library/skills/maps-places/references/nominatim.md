...
`places_search.py` берёт контакт из `NOMINATIM_CONTACT` в `${CODEX_PACK_ROOT}/library/.credentials.master.env`; без него он работает, но пишет предупреждение в stderr. Подставлять чужой или общий адрес нельзя: Nominatim банит по User-Agent целиком, и у всех, кто оставил дефолт, поиск отвалится разом.
