...
Скрипт `${CODEX_PACK_ROOT}/library/skills/edit-banana/scripts/edit-banana.sh` — обёртка,
принимает путь к картинке и вызывает pipeline из корректной директории.

```bash
bash ${CODEX_PACK_ROOT}/library/skills/edit-banana/scripts/edit-banana.sh path/to/diagram.png
