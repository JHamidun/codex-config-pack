...
`python ${CODEX_PACK_ROOT}/library/tools/osint_client.py <команда> [аргументы] [--json]`
...
python ${CODEX_PACK_ROOT}/library/tools/osint_client.py recon example.com

# Чей IP и где стоит
python ${CODEX_PACK_ROOT}/library/tools/osint_client.py ip YOUR_PUBLIC_IP

# Что торчит наружу у хоста + известные CVE
python ${CODEX_PACK_ROOT}/library/tools/osint_client.py ports YOUR_PUBLIC_IP

# Живые субдомены (карта поверхности атаки)
python ${CODEX_PACK_ROOT}/library/tools/osint_client.py subdomains github.com --limit 100

# Гигиена заголовков с оценкой A-F
python ${CODEX_PACK_ROOT}/library/tools/osint_client.py headers https://example.com

# Санкционный скрининг контрагента
python ${CODEX_PACK_ROOT}/library/tools/osint_client.py sanctions "Название Компании"

# Свежие CVE по продукту / конкретный CVE
python ${CODEX_PACK_ROOT}/library/tools/osint_client.py cve "openssl" --limit 5
python ${CODEX_PACK_ROOT}/library/tools/osint_client.py cve CVE-2024-3094

# Машинный вывод для дальнейшей обработки
python ${CODEX_PACK_ROOT}/library/tools/osint_client.py recon example.com --json
```

## Опциональные ключи

Читаются из `${CODEX_PACK_ROOT}/library/.credentials.master.env`. Без них всё работает, просто скромнее:
...
- **OpenSanctions с 2025 отдаёт 401 без ключа.** Фолбэк OFAC качает `sdn.csv` + `alt.csv` (~6 МБ) в `${CODEX_PACK_ROOT}/library/cache/osint/`, TTL 7 дней. Первый вызов — минуту, дальше мгновенно.
