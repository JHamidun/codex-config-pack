...
5. Start container:
   ...
   docker run -d --name openclaw-mybot --init --restart unless-stopped \
     --dns 8.8.8.8 -e HOME=<UPSTREAM_HOME> -e NODE_ENV=production \
     -e OPENAI_API_KEY="..." -e OPENCLAW_GATEWAY_TOKEN="..." \
     -v openclaw-mybot-config:<UPSTREAM_HOME>/.openclaw \
     -v openclaw-mybot-workspace:<UPSTREAM_HOME>/.openclaw/workspace \
