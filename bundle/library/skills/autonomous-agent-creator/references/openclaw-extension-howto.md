...
Extensions live inside the OpenClaw container at `<UPSTREAM_HOME>/.openclaw/extensions/<extension>/`.
...
```dockerfile
COPY extensions/my-extension <UPSTREAM_HOME>/.openclaw/extensions/my-extension
RUN cd <UPSTREAM_HOME>/.openclaw/extensions/my-extension && npm install --production
```
