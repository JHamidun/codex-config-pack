...
**Fix (OpenClaw):** Handled automatically via workspace volume — offset is persisted in `<UPSTREAM_HOME>/.openclaw/state/`.
...
#!/bin/bash
sleep 10
docker exec -u root $1 bash <UPSTREAM_HOME>/.openclaw/workspace/setup_deps.sh
