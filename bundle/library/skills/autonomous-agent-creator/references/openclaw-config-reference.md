...
Or use SOUL.md file at `<UPSTREAM_HOME>/.openclaw/SOUL.md` for longer personality definitions.
...
```json
{
  "skills": {
    "load": {
      "extraDirs": ["<UPSTREAM_HOME>/.openclaw/skills"]
    },
...
Place `SKILL.md` files in `<UPSTREAM_HOME>/.openclaw/skills/<skill-name>/SKILL.md`.
...
Extensions are TypeScript modules placed in `<UPSTREAM_HOME>/.openclaw/extensions/`.
...
|----------|----------|-------------|
| `HOME` | Yes | Must be `<UPSTREAM_HOME>` |
...
| Config path (container) | `<UPSTREAM_HOME>/.openclaw/openclaw.json` |
| Workspace path | `<UPSTREAM_HOME>/.openclaw/workspace/` |
| Skills path | `<UPSTREAM_HOME>/.openclaw/skills/` |
...
| Config not loading | Wrong path or permissions | Must be `<UPSTREAM_HOME>/.openclaw/openclaw.json`, owned by UID 1000 |
...
| Skills not found | Wrong directory | Place in `<UPSTREAM_HOME>/.openclaw/skills/<name>/SKILL.md` |
