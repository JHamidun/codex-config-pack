...
- Mount compiled `dist/` into `<UPSTREAM_HOME>/.openclaw/extensions/my-extension/`.
...
- **Fix hardcoded paths in workspace scripts** — `sed -i 's|<UPSTREAM_HOME>/.openclaw/workspace|/opt/data/workspace|g' /opt/data/workspace/*.py` (OpenClaw used `<UPSTREAM_HOME>/.openclaw/workspace`, Hermes uses `/opt/data/workspace`)
...
until python ${CODEX_PACK_ROOT}/library/tools/tg_client.py read-chat <BotName>_bot --limit 1 \
        | grep -E '<BotName>.*\] ' \
...
MSYS_NO_PATHCONV=1 python ${CODEX_PACK_ROOT}/library/tools/tg_client.py send @BotName "/start"
# Read:
python ${CODEX_PACK_ROOT}/library/tools/tg_client.py read-chat @BotName --limit 3
```
