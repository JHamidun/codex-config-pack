...
ssh "$SERVER" "docker exec -u 0 hermes-<bot>-bootstrap bash -c '
  grep -lE \"<UPSTREAM_HOME>/.openclaw/workspace\" /opt/data/workspace/*.py 2>/dev/null \
    | xargs -I {} sed -i \"s|<UPSTREAM_HOME>/.openclaw/workspace|/opt/data/workspace|g\" {}
'"
```
...
python ${CODEX_PACK_ROOT}/library/tools/tg_client.py send <BotUsername> "тест 1"
sleep 30
python ${CODEX_PACK_ROOT}/library/tools/tg_client.py read-chat <BotUsername> --limit 4
```
