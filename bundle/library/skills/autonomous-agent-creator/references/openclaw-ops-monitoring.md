...
**Symptom:** `read failed: ENOENT: no such file or directory, access '<UPSTREAM_HOME>/.openclaw/workspace/MEMORY.md'`

**Root cause:** Agent tries to read memory file that was never created.

**Fix:**
```bash
docker exec $CONTAINER sh -c 'echo "# Memory\n" > <UPSTREAM_HOME>/.openclaw/workspace/MEMORY.md'
```
...
```bash
# Send message
python ${CODEX_PACK_ROOT}/library/tools/tg_client.py send @BotUsername "test message"

# Read last N messages
python ${CODEX_PACK_ROOT}/library/tools/tg_client.py read-chat @BotUsername --limit 5
...
for bot in $BOTS; do   # BOTS="@bot1 @bot2 …" — свои из fleet inventory
  MSYS_NO_PATHCONV=1 python ${CODEX_PACK_ROOT}/library/tools/tg_client.py send $bot "тест"
done
sleep 30
for bot in $BOTS; do   # BOTS="@bot1 @bot2 …" — свои из fleet inventory
  python ${CODEX_PACK_ROOT}/library/tools/tg_client.py read-chat $bot --limit 1
done
```
...
curl -s -H "Authorization: Bearer $API_KEY" https://api.anthropic.com/v1/messages \
  -d '{"model":"claude-haiku-4-5-20251001","max_tokens":10,"messages":[{"role":"user","content":"hi"}]}' | head -1

# Check OpenAI
curl -s -H "Authorization: Bearer $API_KEY" https://api.openai.com/v1/models | python3 -c 'import json,sys; d=json.load(sys.stdin); print("OK" if "data" in d else d.get("error",{}).get("message","FAIL"))'
```
