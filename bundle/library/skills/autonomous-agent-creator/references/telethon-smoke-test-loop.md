...
When fixing a frozen or misbehaving bot, do NOT loop "send test → ask user → wait → repeat". You can drive Telegram yourself via `${CODEX_PACK_ROOT}/library/tools/tg_client.py` as the user account (Telethon), then read replies, parse them, and iterate fixes autonomously.
...
python ${CODEX_PACK_ROOT}/library/tools/tg_client.py send <BotName>_bot "тест N"

# 2. Wait for a REAL agent reply (not an error/retry/system message)
until python ${CODEX_PACK_ROOT}/library/tools/tg_client.py read-chat <BotName>_bot --limit 1 2>&1 \
        | grep -E '<BotName>.*\] ' \
...
# 3. Show final reply
python ${CODEX_PACK_ROOT}/library/tools/tg_client.py read-chat <BotName>_bot --limit 4
```
...
for bot in <BotName> SecondBot_bot AnotherBot_bot; do
  python ${CODEX_PACK_ROOT}/library/tools/tg_client.py send "${bot}" "тест fleet" &
done
wait
...
for bot in <BotName> SecondBot_bot AnotherBot_bot; do
  echo "=== ${bot} ==="
  until python ${CODEX_PACK_ROOT}/library/tools/tg_client.py read-chat "${bot}" --limit 1 2>&1 \
          | grep -E "${bot}.*\] " \
          | grep -vE 'тест fleet|API call|⏳|⚠|Gateway'; do
    sleep 5
  done
  python ${CODEX_PACK_ROOT}/library/tools/tg_client.py read-chat "${bot}" --limit 3 | tail -10
done
```
...
python ${CODEX_PACK_ROOT}/library/tools/tg_client.py send <BotName> "тест 7"
...
for msg in "привет" "сколько 2+2" "расскажи короткую шутку"; do
  python ${CODEX_PACK_ROOT}/library/tools/tg_client.py send <BotName> "${msg}"
  until python ${CODEX_PACK_ROOT}/library/tools/tg_client.py read-chat <BotName> --limit 1 2>&1 \
          | grep -E '<BotName>.*\] ' \
...
python ${CODEX_PACK_ROOT}/library/tools/tg_client.py read-chat <BotName> --limit 6
```
...
python ${CODEX_PACK_ROOT}/library/tools/tg_client.py send-file <BotName> /path/to/voice.ogg

# Document
python ${CODEX_PACK_ROOT}/library/tools/tg_client.py send-file <BotName> /path/to/budget.xlsx

# Photo (Hermes routes through vision_analyze)
python ${CODEX_PACK_ROOT}/library/tools/tg_client.py send-file <BotName> /path/to/test.jpg
```
