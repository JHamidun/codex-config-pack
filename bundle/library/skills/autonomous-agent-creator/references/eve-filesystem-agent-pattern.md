...
and our own top-level `${CODEX_PACK_ROOT}/library/skills/<name>/SKILL.md` + `references/` + `scripts/`
...
- Eve's directory-per-capability instinct is *why* our own skill library (this whole
  `${CODEX_PACK_ROOT}/library/skills/` tree) works well — it's independent validation of the pattern we
