...
> Цель — усилить локальный граф памяти `${CODEX_PACK_ROOT}/library/scripts/memory_graph.py` (`${CODEX_PACK_ROOT}/library/memory-graph/graph.db`).
...
Вызов:
```bash
python ${CODEX_PACK_ROOT}/library/scripts/memory_graph.py gaps         # порог устаревания 45 дней
python ${CODEX_PACK_ROOT}/library/scripts/memory_graph.py gaps 30      # свой порог
```
