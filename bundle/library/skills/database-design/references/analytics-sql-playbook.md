...
Три движка нашего стека. Всё в SQLite-разделе проверено запусками на `${CODEX_PACK_ROOT}/library/chats.db`
(560 545 сообщений, 75 532 сессии) и `${CODEX_PACK_ROOT}/library/memory-graph/graph.db`, SQLite 3.49.1.
...
```sql
-- ВПИШИ АБСОЛЮТНЫЙ ПУТЬ вместо <HOME>: <UPSTREAM_HOME> <UPSTREAM_HOME> <UPSTREAM_HOME>
