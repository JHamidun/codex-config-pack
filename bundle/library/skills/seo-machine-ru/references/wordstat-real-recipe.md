...
Свои креды — в `${CODEX_PACK_ROOT}/library/.credentials.master.env`: `YANDEX_EMAIL` + `YANDEX_PASSWORD`. Нужен обычный аккаунт Яндекса, платного доступа Wordstat не требует.
...
## Связки
- Чинёный клиент Direct API: `${CODEX_PACK_ROOT}/library/skills/yandex/scripts/yandex_api.py wordstat` (когда дадут доступ).
