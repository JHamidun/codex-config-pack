...
| 2 | **HTTP Request** | Perplexity research | POST к Perplexity API, body с промптом research-агента, заголовок `Authorization: Bearer $API_KEY
...
Все ключи — из `${CODEX_PACK_ROOT}/library/.credentials.master.env` (см. правило security). В n8n — через
