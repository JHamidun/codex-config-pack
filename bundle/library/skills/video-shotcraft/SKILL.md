...
Скилл на китайском. Ниже карта, чтобы не искать вслепую.

**Три режима сборки ролика** (выбрать ДО начала работы, не смешивать):

| Режим | Когда | Что читать |
|-------|-------|------------|
| Шаблон Ink Press | Готовый ролик 36,2 с / 1920×1080 / 10 сцен — подставить свои скриншоты, тексты, бренд | `template/TEMPLATE.md` |
| Автономная сборка | Агент сам решает визуал, раскадровку и звук, идёт до финала без пауз | `references/pipeline.md` |
| Совместная сборка | Агент предлагает — владелец утверждает бриф, направление, раскадровку | `references/guided-free-creation.md` |

Если дан только URL или проект и режим не выбран — сначала **минимальная read-only проверка
продукта** (позиционирование, функции, визуал, что можно показать), затем предложить режим.
Бизнес-проект при этом НЕ править и чувствительные данные НЕ собирать.

**153 карточки-рецепта кадров** — `references/shots/`:

| Категория | Шт | Про что |
|-----------|-----|---------|
| `typography` | 25 | кинетическая типографика, работа с текстом |
| `ui-entrance` | 27 | появление элементов интерфейса |
| `transition` | 19 | переходы между сценами |
| `effects` | 17 | эффекты и обработка |
| `interaction` | 15 | имитация действий пользователя |
| `data` | 11 | графики, счётчики, визуализация цифр |
| `rhythm` | 11 | ритмические монтажные приёмы |
| `camera` | 10 | 2.5D-проходы, движение камеры по странице |
| `opening` | 10 | открывающие кадры |
| `outro` | 7 | финальные кадры, CTA |

**Методология** — `references/`: `aesthetic-rules.md` (24 КБ, визуальные правила),
`sound-design.md` (23 КБ, звук), `music-beat-sync.md` (кадрирование под бит),
`final-review.md` (приёмка готового ролика).

**Связь с остальным конфигом:**

- рендер альфы для монтажной программы, правило Studio-схемы, кадровая анимация →
  `video-generation/references/remotion-overlays.md` (там же ProRes 4444 вместо webm);
- нарезка готового футажа, субтитры, цветокор → `video-editor`;
- вертикальные reels 9:16 с озвучкой → `video-montage`;
- AI-генерация исходного видео (Veo, Sora, Runway) → `video-generation`.

**Лицензия и происхождение.** Apache-2.0. Карточки написаны с нуля по мотивам публичных
промо-роликов (ClickUp, Slack, Notion, Figma) и работ моушн-дизайнеров; исходных
материалов и брендинга в репозитории нет. Подробности — `references/shots/ATTRIBUTION.md`.
