...
**Перед началом прочитай `${CODEX_WORKSPACE}/.codex-context/business-context.md`** — разделы «Воронка» (шаги от первого касания до оплаты), «Экономика» (средний чек, CPL, CAC, LTV, отток) и «Учёт и аналитика» (где источник правды по выручке и связаны ли между собой реклама, CRM и платежи). Файла нет — заведи из `${CODEX_PACK_ROOT}/library/templates/business-context.md`.
