...
из `${CODEX_WORKSPACE}/.codex-context/author-profile.md` (раздел «Площадки»; файла нет — заведи из
`${CODEX_PACK_ROOT}/library/templates/author-profile.md`). Канал не назван и в профиле его нет — спроси,
...
```bash
python ${CODEX_PACK_ROOT}/library/skills/last30days/scripts/last30days.py --agent --quick --topic "AI"   # timeout 300s
```
...
python ${CODEX_PACK_ROOT}/library/skills/video-generation/scripts/nano_banana_keyframes.py shots.json --out keyframes/
...
python ${CODEX_PACK_ROOT}/library/skills/video-generation/scripts/elevenlabs_voiceover.py scene_NN.txt --out voice_NN.mp3

python ${CODEX_PACK_ROOT}/library/skills/video-editor/video_editor.py music-pool          # готовые треки
python ${CODEX_PACK_ROOT}/library/skills/video-generation/scripts/elevenlabs_music.py \
  "upbeat tech background music, 110 BPM, warm synths" --duration-ms 45000 --out music.mp3

python ${CODEX_PACK_ROOT}/library/skills/video-editor/scripts/sfx.py search whoosh --max 5   # SFX через Freesound
```
...
```bash
python ${CODEX_PACK_ROOT}/library/skills/video-editor/video_editor.py concat scene_01.mp4 scene_02.mp4 … --transition fade -o assembled.mp4
python ${CODEX_PACK_ROOT}/library/skills/video-editor/video_editor.py concat-audio voice_01.mp3 … --gaps voice_timestamps.json -o voice_full.mp3
python ${CODEX_PACK_ROOT}/library/skills/video-editor/video_editor.py ducking assembled.mp4 --voice voice_full.mp3 --music music.mp3 --music-volume -18 -o assembled_mixed.mp4
```
...
1. Submagic, если есть `SUBMAGIC_API_KEY` (CLI у навыка нет, дёргай API по его SKILL.md).
   Локальный эквивалент: `python ${CODEX_PACK_ROOT}/library/skills/video-editor/scripts/add_captions.py assembled_mixed.mp4 final_captioned.mp4 --style hormozi`
...
`${CODEX_PACK_ROOT}/library/.youtube-client-secrets.json`; первый запуск сам откроет браузер и сохранит
`${CODEX_PACK_ROOT}/library/.youtube-oauth-token.json`).
