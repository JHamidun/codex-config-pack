...
Ключ берётся из `${CODEX_PACK_ROOT}/library/.credentials.master.env`. Впиши в него **сам ключ**, а не
код на Python:

```bash
# ${CODEX_PACK_ROOT}/library/.credentials.master.env
...
def deepseek_key() -> str:
    ...
    if not key:
        raise RuntimeError(
            'DEEPSEEK_API_KEY не задан. Впиши ключ в ${CODEX_PACK_ROOT}/library/.credentials.master.env '
    ...
    if bogus or not key.startswith('sk-'):
        raise RuntimeError(
            ...
            'без объяснения причины — исправь ${CODEX_PACK_ROOT}/library/.credentials.master.env.'
        )
    return key
```
...
def deepseek_chat(prompt: str, system_prompt: str = None,
    ...
    try:
        ...
        raise RuntimeError(
            ...
            f'Проверь DEEPSEEK_API_KEY в ${CODEX_PACK_ROOT}/library/.credentials.master.env. Ответ: {e}'
        ) from e
