...
**Перед началом прочитай `${CODEX_WORKSPACE}/.codex-context/business-context.md`** — разделы «Продукт», «ICP», «Цены» (действующая линейка и валюта), «Экономика» (средний чек, CAC, LTV, отток) и «Позиционирование» (с кем тебя сравнивают). Файла нет — заведи из `${CODEX_PACK_ROOT}/library/templates/business-context.md`.
