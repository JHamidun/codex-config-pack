...
Если ведёшь граф памяти (`skills/graph-memory`), связи из него можно рисовать: `python ${CODEX_PACK_ROOT}/library/scripts/memory_graph.py neighbors "<узел>"` (или `path`/`hubs`) → распарси узлы/рёбра → сгенерь Excalidraw (узлы=прямоугольники, рёбра=стрелки с подписью типа связи) → визуальный цикл. Так граф «кто связан с X» становится картинкой. База едет пустой и наполняется по ходу твоей работы — на свежей установке команда вернёт ноль узлов, и это не поломка.
