...
Токены — в `${CODEX_PACK_ROOT}/library/.credentials.master.env` → `HIGGSFIELD_ACCESS_TOKEN` (hf_…) + `HIGGSFIELD_REFRESH_TOKEN`.
...
| Employees (sub-agents + flow-skill) | `agents/` + `${CODEX_PACK_ROOT}/library/skills/*` |
...
| Files/Memory (folders) | `${CODEX_PACK_ROOT}/library/projects/.../memory/` + scratchpad |
...
# cwd: <UPSTREAM_HOME>/{chat_id}
```
→ Песочница — Linux-окружение с рабочим каталогом по chat_id (`<UPSTREAM_HOME>/{chat_id}/`): туда `curl`-ом
...
Задача на монтаж, шаги терминала в `<UPSTREAM_HOME>/{chat_id}/`:
