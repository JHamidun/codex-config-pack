...
|---|---|---|
| Orchestrator (выбор LLM: Claude/Gemini/GPT/Grok) | главный цикл Claude Code + agent `orchestrator` + Task subagents (model: inherit-current-codex-model/sonnet/haiku) | ✅ есть |
| Employees (21 sub-agent) | `${CODEX_PACK_ROOT}/library/agents/` (51 агент) + skills | ✅ есть |
| Flow-skills (cinematic-flow, ugc-*-flow, montage…) | `${CODEX_PACK_ROOT}/library/skills/*` (198 скиллов) | ✅ есть (montage→video-editor, audio→elevenlabs/suno, maps→maps-places, pdf/pptx/youtube…) |
...
| Files / Memory (folders per chat) | `${CODEX_PACK_ROOT}/library/projects/<proj>/memory/` + scratchpad | ✅ есть |
