...
`${CODEX_PACK_ROOT}/library/skills/video-generation/engines/higgsfield/bin/hf` — **8.66 МБ, Go-бинарь** (Cobra CLI), статически слинкован,
...
3. Хранит локально (`auth.Save`/`Load`/`Delete`, `WithLock`), у нас в `${CODEX_PACK_ROOT}/library/.credentials.master.env`: `HIGGSFIELD_ACCESS_TOKEN` (hf_…) + `HIGGSFIELD_REFRESH_TOKEN`.
4. `auth.Refresh` → `POST .../refresh` (postRefresh) при 401. Header `Authorization: Bearer $API_KEY
