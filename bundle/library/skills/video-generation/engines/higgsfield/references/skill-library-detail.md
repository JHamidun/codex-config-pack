...
## pdf
Markdown→брендированный PDF (таблицы/списки/картинки/код). **Файлы только в `<UPSTREAM_HOME>/projects/<project>/`** (вне — включая /tmp — удаляется!). Бренд-цвета #4FCEE4/#1A1C1F/#E72930. `gen_pdf.py` (reportlab) → `higgsfield_upload` → CDN URL.
