...
curl -s -X POST -H "Authorization: Bearer $API_KEY" -F "files[]=@./render_final.mp4" "https://api.higgsfield.ai/v1/assets/upload"
# 2-этап S3 presigned: запросить URL → PUT файл
PRESIGNED_DATA=$(curl -s -X POST -H "Authorization: Bearer $API_KEY" -H "Content-Type: application/json" -d '{"filename":"render_final.mp4","content_type":"video/mp4"}' "https://api.higgsfield.ai/v1/assets/presigned-url")
