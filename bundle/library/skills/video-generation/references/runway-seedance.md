...
`RUNWAY_TOKEN_PLACEHOLDER` в `${CODEX_PACK_ROOT}/library/.credentials.master.env`. Refresh через 30 дней:
...
```bash
python ${CODEX_PACK_ROOT}/library/skills/video-generation/scripts/runway_client.py token-status   # разбирает exp в самом JWT
python ${CODEX_PACK_ROOT}/library/skills/video-generation/scripts/runway_client.py profile        # план + кто залогинен (нужна сеть)
```
...
> ```bash
> python ${CODEX_PACK_ROOT}/library/skills/video-generation/scripts/runway_client.py features    # что реально включено на аккаунте
> ```
