...
**Бриф:** 4 «живые обложки» для книги «Example Book Title» Example Author, 9:16, по 5 секунд каждая. Стейкхолдер — [Stakeholder] <user@example.com>.
...
`HEYGEN_AVATAR_ID` / `HEYGEN_VOICE_ID` в `${CODEX_PACK_ROOT}/library/.credentials.master.env`.
...
```bash
python ${CODEX_PACK_ROOT}/library/skills/shorts-pipeline-user/scripts/trigger_word_check.py script.txt
```
...
Пример для Terra: draft [Stakeholder] `<user@example.com>` с 4 ссылками + блок «что хотим узнать».
