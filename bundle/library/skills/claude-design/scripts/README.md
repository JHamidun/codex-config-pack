...
3. Скопировать значения:
   ...
   - `anthropic-device-id`
4. В `${CODEX_PACK_ROOT}/library/.credentials.master.env`:
   ```bash
...
"claude-design": {
  "type": "stdio",
  "command": "python",
  "args": ["<UPSTREAM_HOME>/.claude/skills/claude-design/scripts/claude_design_mcp.py"],
  "env": {
...
Windows-форма: `<UPSTREAM_HOME>/.claude/skills/claude-design/scripts/claude_design_mcp.py`.
