...
**Перед началом прочитай `${CODEX_WORKSPACE}/.codex-context/business-context.md`** — разделы «Продукт» (чем отличаешься и что альтернативы делают лучше), «Позиционирование» (в какой категории тебя ищут и с кем сравнивают) и «Цены». Файла нет — заведи из `${CODEX_PACK_ROOT}/library/templates/business-context.md`.
