...
Ключи живут в `${CODEX_PACK_ROOT}/library/.credentials.master.env` (создаётся из
`${CODEX_PACK_ROOT}/library/templates/.credentials.master.env.example`), читаются через `os.getenv`.
...
```bash
python ${CODEX_PACK_ROOT}/library/skills/yandex/scripts/yandex_api.py metrika report --metrics "ym:s:visits,ym:s:pageviews" --date1 "7daysAgo" --date2 "today"
python ${CODEX_PACK_ROOT}/library/skills/yandex/scripts/yandex_api.py disk ls /
python ${CODEX_PACK_ROOT}/library/skills/yandex/scripts/yandex_api.py iot devices
python ${CODEX_PACK_ROOT}/library/skills/yandex/scripts/yandex_api.py webmaster sites
```
