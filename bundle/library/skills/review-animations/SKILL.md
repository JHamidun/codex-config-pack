...
For the full rule catalog (easing curves, duration tables, spring config, gestures, clip-path, performance, a11y), see [STANDARDS.md](STANDARDS.md). Load it whenever a finding needs a precise value or citation. STANDARDS.md — единый каталог значений и для emil-design-eng (там философия/решения, цифры — здесь).
