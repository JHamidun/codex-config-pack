...
Скилл закрывает **обе** границы безопасности вокруг авто-загружаемого каталога `${CODEX_PACK_ROOT}/library/`:
...
# macOS / Linux
cp ${CODEX_PACK_ROOT}/library/skills/leak-scan/scripts/identity.example.json ${CODEX_PACK_ROOT}/library/leak-scan-identity.json
```

Открой копию и впиши СВОИ признаки вместо заглушек (`YOURNAME`, `user@example.com`,
...
(путь в переменной окружения) → `${CODEX_PACK_ROOT}/library/leak-scan-identity.json` → `identity.local.json`
...
```bash
python ${CODEX_PACK_ROOT}/library/skills/leak-scan/scripts/leak_scan.py <dir-or-file> [--allow SUBSTR ...]
```
...
python ${CODEX_PACK_ROOT}/library/skills/leak-scan/scripts/leak_scan.py ./my-public-repo
```
...
- **Личные признаки** (твоё имя, домен, клиент) → в `${CODEX_PACK_ROOT}/library/leak-scan-identity.json`, а НЕ в код скрипта. Иначе сканер снова станет носителем утечки.
