...
Drafts a reply to a specific LinkedIn comment. Correctly handles LinkedIn's 2-level thread flattening: if you're replying to a reply, the your social API needs the TOP-level comment URN as `parentComment`, not the reply's URN.
...
- Approval card → on user "post", fires reaction + reply via SocialPublisher
...
7. **On approval — adapt to the active backend.** Call `lib.active_backend()`:
   - **`socialpublisher`** (SOCIALPUBLISHER_API_KEY set) → react on the specific comment being replied to, pause 8-15s, then post reply with the correct top-level `parentComment` URN.
