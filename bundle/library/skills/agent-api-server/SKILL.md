...
`${CODEX_PACK_ROOT}/library/tools/agent_api_server.py` — stdlib-only HTTP-сервер, который принимает
...
# 2. Проверка
python ${CODEX_PACK_ROOT}/library/tools/agent_api_server.py test --model haiku

# 3. Запуск
python ${CODEX_PACK_ROOT}/library/tools/agent_api_server.py serve --port 8199
```

### Env-переменные (значения заполняет владелец; читаются из `${CODEX_PACK_ROOT}/library/.credentials.master.env`)
...
| `AGENT_API_WORKDIR` | нет | cwd для CLI (иначе `${CODEX_PACK_ROOT}/library/agent-api-workdir`) |
...
(реальный UUID сессии CLI). `Authorization: Bearer $API_KEY
...
print(c.chat.completions.create(model: inherit-current-codex-model,
      messages=[{"role":"user","content":"Reply with exactly: SDKOK"}]).choices[0].message.content)

for chunk in c.chat.completions.create(model: inherit-current-codex-model, stream=True,
...
2. **Флаги `--system` и `--max-tokens` в `claude` 2.1.207 не существуют.** Правильные —
   ...
   вообще, длину задавай в промпте. Обёртка `${CODEX_PACK_ROOT}/library/tools/claude_cli.py` этой ошибкой болела
...
10. **Сессии живут в CLI, не в сервере.** `X-Session-Id` → UUIDv5 → `--session-id` (1-й ход) /
    `--resume` (далее); реестр `${CODEX_PACK_ROOT}/library/agent_api_sessions.json`. Если удалить реестр — следующий
...
- [ ] `python ${CODEX_PACK_ROOT}/library/tools/agent_api_server.py test --model haiku` → ALL CHECKS PASSED
