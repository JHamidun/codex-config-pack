...
### Пунктуация
Правило точки в тултипах и подписях — см. §L «Типографика».
...
### Плейсхолдер
1. Показывает **пример** заполнения, не инструкцию. Хорошо: `John Doe`. Плохо: `Введите имя`.
...
8. Называйте формат: «email в формате user@example.com».
...
|---|---|---|---|
| Email|Электронная почта|user@example.com|На эту почту придёт подтверждение |
| Телефон|Телефон|+1234567890| |
| Пароль|Пароль| |Минимум 8 символов, хотя бы одна цифра |
| ИНН|ИНН|7700000000|10 цифр для юрлица, 12 для ИП |
...
| Дашборд vs панель|«Дашборд» прижился в продуктовых интерфейсах; «панель»/«сводка» — допустимая альтернатива по контексту |
...
7. **Кнопки:** «Зарегистрироваться» (18) vs «Sign up» (7) — закладывайте запас ширины.
...
2. В подписях (тосты, хинты, тултипы): точка только между предложениями, не в конце. Исключение — полноценный текст из 2+ абзацев: обычные правила пунктуации.
