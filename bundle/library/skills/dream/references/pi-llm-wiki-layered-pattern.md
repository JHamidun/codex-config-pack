...
Our memory (`${CODEX_PACK_ROOT}/library/rules/auto-learning.md` + `${CODEX_PACK_ROOT}/library/skills/dream/`) already
...
|---|---|---|
| Layer 1 — immutable source packets | `chats.db` (FTS5 session archive) + `${CODEX_PACK_ROOT}/library/projects/*/archive/` raw transcripts | **We don't wrap raw captures in a manifest+extracted.md packet with a stable SRC-id.** Our raw layer is a searchable DB, not addressable per-artifact files a memory note can cite by ID. |
...
| Layer 3 — auto metadata (registry/backlinks/events) | `memory_graph.py` (`${CODEX_PACK_ROOT}/library/memory-graph/graph.db`) — notes+[[links]]+supersedes graph, `dangling`/`orphans` health checks | Close parallel: `memory_graph.py dangling`/`orphans` ≈ `wiki_lint`. **We don't have an append-only `events.jsonl`** — dream's own actions (what got superseded/pruned/promoted, when) live only in git-less file diffs, not a durable event log a future dream pass could query ("show me every prune in the last month"). |
| Layer 4 — config | `rules/auto-learning.md` + `memory-v2.md` regulation itself | Direct match, no gap. |
| Two-tier vault (personal/project) | Global memory dir vs `${CODEX_PACK_ROOT}/library/projects/<project>/memory/` | Direct match already — same personal-follows-everywhere + project-opt-in split, just not named "vault". |
...
- A literal `.llm-wiki/` directory structure / Obsidian plugin install is not warranted.
  ...
  a separate concern from `${CODEX_PACK_ROOT}/library/projects/*/memory/` — adding a second wiki mechanism
