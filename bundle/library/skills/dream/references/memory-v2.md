...
> Canonical reference for the v2 memory regulation referenced from `${CODEX_PACK_ROOT}/library/rules/auto-learning.md`
...
Memory directory (global): `${CODEX_PACK_ROOT}/library/projects/C--Users-youruser/memory/`
Project memories: `${CODEX_PACK_ROOT}/library/projects/<project>/memory/`
...
Action: copy the note into the global memory dir (`${CODEX_PACK_ROOT}/library/projects/C--Users-youruser/memory/`), retype as `user`/`feedback`, add an index line in global `MEMORY.md`. In the project copy set `status: promoted` + a `promoted_to:` pointer (or leave it if the project index still needs it locally).
