...
Скрипту нужен `OPENAI_API_KEY` (окружение или `${CODEX_PACK_ROOT}/library/.credentials.master.env` — см. `scripts/config.py`). Модель — дешёвая mini-линейка, ~$0.0005 за ролик; 87 нарезок ≈ $0.05 на анализ. Запускать по мере появления новых SRT в `$SHORTS_SOURCE/`.
