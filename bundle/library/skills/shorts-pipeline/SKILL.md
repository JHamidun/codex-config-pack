...
export YOUTUBE_TOKEN_FILE=${CODEX_PACK_ROOT}/library/.youtube-oauth-token.json   # умолчание
```

Умолчание `SHORTS_HOME` — `./shorts-pipeline` в текущем каталоге. Ключи берутся из
окружения, а если рядом лежит `${CODEX_PACK_ROOT}/library/.credentials.master.env` — то и оттуда
...
python ${CODEX_PACK_ROOT}/library/skills/shorts-pipeline/scripts/heygen_avatar_v.py --check-wallet

# Шаг 1 — нарезка SRT в фабричной папке + GPT анализ
python ${CODEX_PACK_ROOT}/library/skills/shorts-pipeline/scripts/analyze_srt.py

# Шаг 2 — сделать один shorts из конкретного key (mode tts-clean — default)
python ${CODEX_PACK_ROOT}/library/skills/shorts-pipeline/scripts/full_pipeline.py 0HCAhzgS27Y/short_01

# Шаг 3 — batch первых 5 viable (пакет платный → --yes обязателен, сперва печатается оценка)
python ${CODEX_PACK_ROOT}/library/skills/shorts-pipeline/scripts/full_pipeline.py --batch 5 --mode tts-clean --yes

# Шаг 4 — весь пакет (на сутки работы HeyGen/SubMagic); 81 ролик ≈ $162
python ${CODEX_PACK_ROOT}/library/skills/shorts-pipeline/scripts/full_pipeline.py --all --mode tts-clean --yes

# Mode real-audio (свой живой голос вместо TTS) — нужна папка с mp3 на каждый key
python ${CODEX_PACK_ROOT}/library/skills/shorts-pipeline/scripts/full_pipeline.py --all --mode real-audio --audio-dir ~/voices/ --yes

# Cleanup канала (удаление необратимо → снимок не старше 24 ч + --yes)
python ${CODEX_PACK_ROOT}/library/skills/shorts-pipeline/scripts/inventory.py
python ${CODEX_PACK_ROOT}/library/skills/shorts-pipeline/scripts/cleanup.py --action plan
python ${CODEX_PACK_ROOT}/library/skills/shorts-pipeline/scripts/cleanup.py --action delete --yes --limit 50

# Stats канала
python ${CODEX_PACK_ROOT}/library/skills/shorts-pipeline/scripts/stats.py
```
...
```
${CODEX_PACK_ROOT}/library/skills/shorts-pipeline/
├── SKILL.md                       # this file
├── scripts/
