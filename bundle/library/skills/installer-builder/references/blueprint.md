...
If the config is a GitHub repo with its own `install.ps1/.sh`: the config script prefers the **bundled** copy (`$HM_BUNDLED_CONFIG/install.ps1`, offline) else `git clone` (online), runs it, then applies pack pruning, then verifies `${CODEX_PACK_ROOT}/library` exists. Wrap the nested installer in `try/catch` (it may have its own `Stop`).
