...
- **Cut background** вручную (если картинка не через wrapper; gpt-image-2 НЕ умеет прозрачный фон):
  ```bash
  python ${CODEX_PACK_ROOT}/library/skills/cards-creator/scripts/cut_bg.py name.png   # -> name_t.png  (rembg isnet + alpha matting)
  ```
...
**4. Render + eyeball:**
```bash
python ${CODEX_PACK_ROOT}/library/skills/cards-creator/scripts/render_cards.py series.html   # -> ./png/series-NN.png
```
...
```bash
python ${CODEX_PACK_ROOT}/library/tools/tg_bot.py --token MYBOT --dry-run album --to @yourchannel \
...
```bash
python ${CODEX_PACK_ROOT}/library/skills/cards-creator/scripts/build_story_frames.py        # png/series-* -> story_png/story-*
python ${CODEX_PACK_ROOT}/library/skills/cards-creator/scripts/post_stories.py list your_username   # лимит = boost level!
python ${CODEX_PACK_ROOT}/library/skills/cards-creator/scripts/post_stories.py post your_username
