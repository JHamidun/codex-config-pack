...
```
${CODEX_PACK_ROOT}/library/skills/cards-creator/     ← read-only
...
**НЕ создавай серию внутри `${CODEX_PACK_ROOT}/library/skills/cards-creator/`** — скилл общий и read-only,
...
python ${CODEX_PACK_ROOT}/library/skills/cards-creator/scripts/render_cards.py series.html
...
Альбом шлёт бот, `${CODEX_PACK_ROOT}/library/tools/tg_bot.py album` (навык `tg-bot-publish`). Бот должен быть
...
`--token` принимает и сам токен, и имя переменной из `${CODEX_PACK_ROOT}/library/.credentials.master.env`.
