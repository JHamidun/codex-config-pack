...
- Резолвить ссылки вида `series:slug` через реестр серий
  `${CODEX_PACK_ROOT}/library/skills/habr-post/templates/series.json` (canonical URL).
- Хабы и теги генерируются из topics по `${CODEX_PACK_ROOT}/library/skills/habr-post/templates/habr_footer.md`.
