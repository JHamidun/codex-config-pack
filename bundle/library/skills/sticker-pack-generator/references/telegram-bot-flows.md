...
cred = open(os.path.expanduser('${CODEX_PACK_ROOT}/library/.credentials.master.env'), encoding='utf-8').read()
API_ID = int(cred.split('TELEGRAM_API_ID=')[1].split('\n')[0].strip())
API_HASH = cred.split('TELEGRAM_API_HASH=')[1].split('\n')[0].strip()
SESSION = os.path.expanduser('${CODEX_PACK_ROOT}/library/telegram_session')
...
Файл `${CODEX_PACK_ROOT}/library/telegram_session.session` это SQLite — если два процесса откроют — `database is locked`. Решение:
