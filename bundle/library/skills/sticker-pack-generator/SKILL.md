...
All upload scripts use Telethon, share `scripts/_telethon_base.py`, and key off `${CODEX_PACK_ROOT}/library/.credentials.master.env` (`TELEGRAM_API_ID`, `TELEGRAM_API_HASH`). Session file: `${CODEX_PACK_ROOT}/library/telegram_session`.
...
```
scripts/
├── _config.py                  — loads creds from ${CODEX_PACK_ROOT}/library/.credentials.master.env
