...
> **Источник истины: `${CODEX_PACK_ROOT}/library.json` → `mcpServers`** и `/mcp` в сессии.
> ⚠️ Секция `mcpServers` в `${CODEX_PACK_ROOT}/library/settings.json` **не читается вовсе** — молча, без ошибки
> и без строки в логе. Серверы при этом выглядят настроенными и не поднимаются ни разу.
> Пак кладёт свои в `${CODEX_PACK_ROOT}/library.json` скриптом `scripts/mcp_install.py` (шаблон —
> `templates/mcp-servers.json`); твои одноимённые он не трогает, свои помнит в
> `${CODEX_PACK_ROOT}/library/.ccpack-mcp.txt`. Проверять правку, не перезапуская сессию: `claude mcp list`
> из нового процесса.
...
- ⚠️ pageindex через `npx @pageindex/mcp` — это ОБЛАКО VectifyAI: нужен отдельный PAGEINDEX_API_KEY и документы уходят наружу. Локальный движок работает на OPENAI_API_KEY и данные с машины не выпускает — он и есть основной путь (skill `research-docs`). В пак он не входит (это клон стороннего репозитория со своим venv): `git clone https://github.com/VectifyAI/PageIndex ${CODEX_PACK_ROOT}/library/mcps/pageindex` + venv по их README, свою обёртку `pi.py` (index/tree/pages/ask) написать поверх.
