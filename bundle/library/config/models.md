...
|-------|----------|----------|------|
| `model: inherit-current-codex-model` | `claude-fable-5-1` | Claude Fable 5.1 | **Дефолт ВСЕХ text-субагентов**; вышла 01.09.2026 |
| `model: inherit-current-codex-model` | `claude-opus-5` | Claude Opus 5 | Оркестратор / основная сессия |
| `model: inherit-current-codex-model` | `claude-sonnet-5` | Claude Sonnet 5 | Доступен, но для text-субагентов НЕ дефолт |
| `model: inherit-current-codex-model` | `claude-haiku-4-5-20251001` | Claude Haiku 4.5 | Быстрые/простые операции, классификация |
...
- Дефолт text-воркеров — **Fable 5.1** (`model: inherit-current-codex-model`). Промптить намерениями
...
Task(subagent_type="general-purpose", model: inherit-current-codex-model, prompt="...")
...
> отдаёт только браузер: на WebFetch она возвращает 403):
>
> | | |
> |---|---|
...
Все ключи: `${CODEX_PACK_ROOT}/library/.credentials.master.env`