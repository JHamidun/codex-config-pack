...
- Session files: `${CODEX_PACK_ROOT}/library/projects/<project>/`
...
- [ ] Прогнать сканер секретов по всему дереву — **0 находок** до любого push. Публичные инструменты:
      `gitleaks detect --source ${CODEX_PACK_ROOT}/library --no-git --redact -v` (быстрый, есть в brew/scoop/apt)
      или `trufflehog filesystem ${CODEX_PACK_ROOT}/library --results=verified,unknown`.
