...
> Pre-configured Claude Code environment: сотни skills, 75 agents, 156 commands, 33 plugins, MCP-серверы в `settings.json` + справочник `mcp.json`. Точные счётчики не вкапываем — они устаревают молча; актуальные: `python ${CODEX_PACK_ROOT}/library/scripts/config_health.py`.
...
1. Copy the example file:
   ```bash
   cp ${CODEX_PACK_ROOT}/library/templates/.credentials.master.env.example ${CODEX_PACK_ROOT}/library/.credentials.master.env
   ```
...
Expected: all rules from `${CODEX_PACK_ROOT}/library/rules/` auto-loaded; MCP `graph-memory`, `filesystem`, `playwright-live1` active.
...
`${CODEX_PACK_ROOT}/library/mcp.json` — СПРАВОЧНИК из 17 готовых блоков (postgres, redis,
...
|---------|-------|------------|
| **Skills** | `${CODEX_PACK_ROOT}/library/skills/*/SKILL.md` | Prompt templates — enhance Claude's domain expertise |
| **Agents** | `${CODEX_PACK_ROOT}/library/agents/**` (75) | Specialized subagents (code-reviewer, bug-hunter, test-writer) — auto-delegated by complexity |
| **Commands** | `${CODEX_PACK_ROOT}/library/commands/**` (156) | Slash commands (`/deploy`, `/translate`, `/gmail`) — shortcuts for common workflows |
| **Rules** | `${CODEX_PACK_ROOT}/library/rules/` | Auto-loaded files — guidelines Claude follows every session |
...
> Счётчики устаревают молча — сверяй `python ${CODEX_PACK_ROOT}/library/scripts/config_health.py`, а не цифры в документах.
...
3. **Memory persists**: important decisions, bug fixes, and patterns are auto-saved to `${CODEX_PACK_ROOT}/library/projects/*/memory/`
...
| "API key not found" | Verify `${CODEX_PACK_ROOT}/library/.credentials.master.env` has the key, loaded via `os.getenv()` |
...
| Telegram tools fail | Run `python ${CODEX_PACK_ROOT}/library/tools/tg_client.py` once to authenticate Telethon session |
