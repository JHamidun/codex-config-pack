# slack-gif-creator: Codex execution

Compose animation frames with native tools or reviewed task-specific code. Build looping GIF with actual transparency, dimensions, duration and byte-budget validation. Review the animation, not just its first frame.

1. Read `${CODEX_PACK_ROOT}/native/OPTIONAL-ADAPTERS.md`, the **gif** section and shared execution/setup contract. Only read the relevant section plus the shared boundaries.
2. Preserve domain methodology from `library/skills/slack-gif-creator/SKILL.md` and its explicitly needed references through `catalog.py --read-reference`. Historical tool calls, provider defaults and `.source` files are not executable.
