# autocad-com: Codex execution

Attach to the running user-selected CAD application. Read-only inspection first. Only create a new drawing; preserve existing documents. Read back entities and inspect measurements.

1. Read `${CODEX_PACK_ROOT}/native/OPTIONAL-ADAPTERS.md`, the **cad-inspect, cad-create** section and shared execution/setup contract. Only read the relevant section plus the shared boundaries.
2. Preserve domain methodology from `library/skills/autocad-com/SKILL.md` and its explicitly needed references through `catalog.py --read-reference`. Historical tool calls, provider defaults and `.source` files are not executable.
