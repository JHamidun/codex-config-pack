...
def verified_text(root, relative, manifest):
    path = root / relative
    ...
    return raw.decode('utf-8').replace('${CODEX_PACK_ROOT}', root.resolve().as_posix())


def prepare(root, selector):
...
def main():
    ...
    if args.prepare:
        try:
            result = prepare(args.root, args.prepare)
